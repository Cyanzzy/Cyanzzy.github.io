...
给定一个数组 `arr`，和一个数 `num`，请把小于 `num` 的数放在数组的左边，等于 `num` 的数放在数组的中间，大于 `num` 的数放在数组的右边。要求额外空间复杂度O(1)，时间复杂度 O(N)。
> 算法思想

i表示小于区域的右边界，k表示大于区域的左边界

* $arr[index]<num$，arr[index]和小于区域i下一个交换，i++，index++
* $arr[index]=num$，index++
* $arr[index]>num$，arr[index]和大于区域k前一个交换，k--，index不变（这时候前一个数是刚过来的，index不能动）
...
public int[] Netherlands(int[] arr, int left, int right, int target) {
    int i = left - 1; // i表示小于区域的右边界
    int k = right + 1; // k表示大于区域的左边界

    while (left < k) {
        if (arr[left] < target) {
            swap(arr, ++i, left++); // 如果当前数小于num，当前数和小于区域下一个数交换，小于区域右阔，j++
    ...
    // target边界
    return new int[] {i + 1, k - 1}; // 等于区域的左右边界
}
```
...
**情况4**
此时`arr[0]` ~ `arr[1]`是下降趋势, `arr[mid-1]`~`arr[mid]`是上升趋势，所以在此范围内数组`arr`一定存在局部最小。去掉`mid`右边的范围，直接找`mid`左边。
...
**情况6**
此时`arr[0]`~`arr[1]`是下降趋势， a`rr[mid-1]`~`arr[mid]`是上升趋势，所以在此范围内数组`arr`一定存在局部最小。去掉`mid`右边的范围，找`mid`左边。
